    ...
    <View className="flex-1 bg-[#f4f4f5]" style={{ paddingTop: insets.top }}>
      <View className="px-5 pb-3 pt-3">
        <View className="flex-row items-center justify-between">
          <View>
            <Text className="text-[30px] font-black tracking-tight">
              Discover
            </Text>
            <Text className="mt-0.5 text-sm text-[#777]">
              Explore Hook&apos;s commercial catalog
            </Text>
          </View>
        ...
        ListHeaderComponent={
          <View className="pb-2">
            ...
            >
              ...
              {categories.map((category) => (
                <FilterChip
                  ...
                  label={category.name}
                  active={categoryId === category.publicId}
            ...
            <View className="mb-3 mt-6 flex-row items-center justify-between">
              ...
              <Text className="text-xs font-semibold text-[#777]">
          ...
          !loading ? (
            <View className="mt-16 items-center px-8">
              <View className="h-16 w-16 items-center justify-center rounded-full bg-white">
                <Ionicons name="search-outline" size={29} color="#999" />
              </View>
              <Text className="mt-4 text-lg font-black">Nothing found</Text>
...
function FilterChip({
  ...
  return (
    <Pressable
      onPress={onPress}
      className={`h-10 justify-center rounded-full border px-4 ${active ? "border-black bg-black" : "border-black/5 bg-white"}`}
    >
      <Text
        numberOfLines={1}
        className={`text-xs font-bold ${active ? "text-white" : "text-[#555]"}`}
      >
        {label}
      </Text>
    </Pressable>
...
function ProductCard({
  ...
  product: import("@/lib/mobile-api").PublicCatalogProduct;
}) {
  return (
    <Pressable
      onPress={() =>
      ...
      className="flex-1 overflow-hidden rounded-[20px] bg-white p-2.5"
    >
      <View className="relative aspect-square overflow-hidden rounded-[15px] bg-[#eee]">
      ...
      <Text numberOfLines={1} className="mt-1 text-[11px] text-[#777]">
        {product.category?.name || "Hook marketplace"}
      </Text>
    </Pressable>
  );
}
