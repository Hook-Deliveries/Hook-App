...
export default function AddressesScreen() {
  ...
  async function saveAddress() {
    if (!draft.stateId || !draft.localGovernmentAreaId) {
      toast.error("Choose your delivery State and Local Government Area");
      return;
    }
    if (!draft.recipientName.trim() || !draft.phone.trim() || !draft.line1.trim()) {
      toast.error("Add the recipient name, phone number, and house address");
      return;
    }
  ...
  return (
    <View className="flex-1 bg-[#f4f4f5]" style={{ paddingTop: insets.top }}>
        ...
        <View className="gap-3">
          {addressRows.map((address) => (
            <View key={address.publicId} className="rounded-[22px] bg-white p-4">
              ...
              {!address.isDefault ? <Pressable accessibilityRole="button" onPress={() => void makeDefault.mutateAsync(address.publicId).then(() => toast.success("Default address updated")).catch(() => toast.error("Could not update default address"))} className="mt-4 self-start rounded-full bg-[#FFF4C7] px-3 py-2"><Text className="text-xs font-bold text-[#665000]">Make default</Text></Pressable> : null}
            </View>
          ))}
        </View>
      ...
      <BottomSheetModal visible={wizardOpen} onClose={() => setWizardOpen(false)} title={editingId ? "Edit delivery address" : "Add delivery address"} fullScreen accessibilityLabel="Delivery address wizard">
        <View className="flex-1">
          ...
          <ScrollView className="flex-1" automaticallyAdjustKeyboardInsets contentContainerStyle={{ paddingBottom: 20 }} keyboardDismissMode="interactive" keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
...
function AddressStep({ draft, onChange }: { draft: AddressDraft; onChange: <K extends keyof AddressDraft>(key: K, value: AddressDraft[K]) => void }) {
  return <View><Text className="text-2xl font-black text-black">Add delivery details</Text><Text className="mt-2 text-sm leading-5 text-black/55">Tell us where to deliver and who should receive the order. Your State, capital, and LGA are already selected.</Text><View className="mt-5 rounded-2xl border border-black/10 bg-[#fff9df] p-4"><Text className="text-xs font-bold uppercase tracking-wider text-black/45">Delivery area</Text><Text className="mt-2 font-black text-black">{draft.localGovernmentArea}, {draft.cityName}</Text><Text className="mt-1 text-sm text-black/55">{draft.stateName}</Text></View><Text className="mb-2 mt-6 text-sm font-black text-black">Recipient details</Text><View className="gap-3"><TextInput className={inputClass} placeholder="Address label, e.g. Home" placeholderTextColor="#999" value={draft.label} onChangeText={(value) => onChange("label", value)} /><TextInput className={inputClass} placeholder="Recipient name" placeholderTextColor="#999" value={draft.recipientName} onChangeText={(value) => onChange("recipientName", value)} autoCapitalize="words" /><TextInput className={inputClass} placeholder="Phone number" placeholderTextColor="#999" keyboardType="phone-pad" value={draft.phone} onChangeText={(value) => onChange("phone", value)} /><TextInput className="min-h-[72px] rounded-2xl border border-black/10 bg-[#fafafa] px-4 py-3 text-sm" placeholder="House number and street address" placeholderTextColor="#999" value={draft.line1} onChangeText={(value) => onChange("line1", value)} multiline textAlignVertical="top" /><TextInput className={inputClass} placeholder="Apartment, floor or suite (optional)" placeholderTextColor="#999" value={draft.line2 || ""} onChangeText={(value) => onChange("line2", value)} /><TextInput className={inputClass} placeholder="Nearest landmark (optional)" placeholderTextColor="#999" value={draft.landmark || ""} onChangeText={(value) => onChange("landmark", value)} /><TextInput className={inputClass} placeholder="Postcode (optional)" placeholderTextColor="#999" value={draft.postalCode || ""} onChangeText={(value) => onChange("postalCode", value)} keyboardType="numeric" /></View></View>;
}
