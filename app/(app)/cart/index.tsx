...
export default function CartScreen() {
  ...
  return <View className="flex-1 bg-[#f4f4f5]" style={{ paddingTop: insets.top }}>
    ...
    <ScrollView refreshControl={<RefreshControl refreshing={cart.isRefetching} onRefresh={cart.refetch} tintColor="#111" />} contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 180 }}>
      <Pressable onPress={continueShopping} className="flex-row items-center gap-3 rounded-[22px] bg-[#171717] p-4"><View className="h-12 w-12 items-center justify-center rounded-xl bg-white/10"><Ionicons name="bag-handle-outline" size={22} color="#FFC809" /></View><View className="min-w-0 flex-1"><Text numberOfLines={1} className="font-black text-white">Hook marketplace</Text><Text numberOfLines={1} className="mt-1 text-xs text-white/60">Continue browsing the commercial catalog</Text></View><Ionicons name="chevron-forward" size={19} color="#fff" /></Pressable>
...
function EmptyCart() { const insets = useSafeAreaInsets(); return <View className="flex-1 items-center justify-center bg-[#f4f4f5] px-8" style={{ paddingTop: insets.top }}><View className="h-20 w-20 items-center justify-center rounded-full bg-white"><Ionicons name="bag-outline" size={36} color="#aaa" /></View><Text className="mt-5 text-xl font-black">Your cart is empty</Text><Text className="mt-2 text-center text-sm leading-5 text-[#777]">Products you add from Hook’s commercial catalog will appear here.</Text><Pressable onPress={() => router.replace('/(tabs)/location')} className="mt-6 rounded-full bg-black px-5 py-3.5"><Text className="font-black text-white">Discover products</Text></Pressable></View>; }
